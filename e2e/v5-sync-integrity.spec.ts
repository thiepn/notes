...
test('offline startup retains the canonical sign-in and automatically reconnects', async ({ page }) => {
  const rows: RemoteSyncRecord[] = [];
  await cloud(page, rows);
  await page.addInitScript((value) => {
    localStorage.setItem('sb-hycegznamzjhwinegaai-auth-token', JSON.stringify(value));
    localStorage.setItem('notes.supabase.session.v1', JSON.stringify({ ...value, access_token: 'legacy' }));
    let online = false;
